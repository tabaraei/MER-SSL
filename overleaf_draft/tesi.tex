...
Taken together, these results support a narrow but well-evidenced claim: a frozen SSL backbone can anchor an emotion recogniser that is competitive on the harder valence axis among comparable audio-only methods \emph{while} carrying an explanation layer that none of those methods provide. The larger architectural bets --- that adding more encoders, or letting the model learn which layers matter, would bring large gains --- did not pay off, and the thesis says so plainly rather than glossing over it.
