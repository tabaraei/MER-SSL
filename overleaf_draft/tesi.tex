...
The natural-language explanations are produced by a retrieval-augmented generation step. The structured, evidence-grounded prompt assembled from a query and its retrieved neighbours is passed to a Llama~3.2~\cite{llama3_2024} model served locally through Ollama. Running the language model locally keeps the explanation stage self-contained and avoids transmitting any data to an external service, at the cost of the smaller capacity of a locally hosted model. The retrieval result and its grounded prompt are deterministic; the language model only rephrases that evidence in prose.
