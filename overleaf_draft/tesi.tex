...
\begin{tabular}{llccl}
\toprule
\textbf{Method / Architecture} & \textbf{Year} & \textbf{Valence $R^{2}$} & \textbf{Arousal $R^{2}$} & \textbf{Data / notes} \\
\midrule
\multicolumn{5}{l}{\textit{Prior work --- single-dataset, PMEmo only}}\\
Zhang et al., IS13 + SVR/MLR~\cite{zhang_pmemo_2018} & 2018 & ${\sim}0.41^{*}$ & ${\sim}0.52^{*}$ & baselines; $^{*}r^{2}$-derived \\
EmoMucs C1D-M~\cite{deberardinis_emomucs_2020}       & 2020 & 0.349 & 0.557 & 1D CNN; labels $[-1,1]$ \\
EmoMucs C2D-M~\cite{deberardinis_emomucs_2020}       & 2020 & 0.414 & 0.610 & 2D CNN; labels $[-1,1]$ \\
AutoML openSMILE~\cite{simonetta_joint_2024}         & 2024 & 0.525 & 0.727 & hand-crafted \\
Music2Emo~\cite{kang_towards_2025}                   & 2025 & 0.536 & 0.777 & MERT + chord/key \\
Ching \& Widmer, Jukebox + MLP~\cite{ching_data_2025} & 2025 & 0.51 & 0.72 & deep embedding; in-dist.$^{\dagger}$ \\
\midrule
\multicolumn{5}{l}{\textit{Prior work --- additional / cross-domain training data}}\\
Music2Emo, multi-task~\cite{kang_towards_2025}       & 2025 & 0.547 & 0.794 & + 4 datasets \\
AutoML--Joint~\cite{simonetta_joint_2024}            & 2024 & 0.780 & 0.861 & PMEmo + IADS-E (joint) \\
\midrule
\multicolumn{5}{l}{\textit{This work (PMEmo only, audio)}}\\
\textbf{Triple (MERT + w2v2 + mel-CNN)}              & 2026 & \textbf{0.576} & 0.702 & best PMEmo-only valence \\
\textbf{Enhanced (+ tempo/key)}                      & 2026 & 0.569 & \textbf{0.718} & our best arousal \\
\bottomrule
\end{tabular}
\caption{Comparison with prior work on PMEmo, grouped by the data each method uses. Among single-dataset PMEmo-only methods, the proposed Triple model leads on valence ($0.576$); on arousal the system is competitive but trails. The cross-domain AutoML--Joint result ($0.780/0.861$) is the overall bar but relies on joint training with environmental sounds --- the same strategy that fails for self-supervised features (Section~\ref{sec:results-iadse}). The distinguishing contribution is ante-hoc explainability, not an accuracy record. $^{*}$Zhang et al. report Pearson's $r$; entries are squared approximations ($R^{2}\approx r^{2}$). EmoMucs uses labels scaled to $[-1,1]$, so its $R^{2}$ is not perfectly commensurable. $^{\dagger}$The Ching \& Widmer value is the in-distribution ($\text{PMEmo}\!\rightarrow\!\text{PMEmo}$) diagonal of their cross-dataset matrix (Jukebox embeddings + MLP, $R^{2}$); the proposed Triple model leads it on valence ($0.576$ vs.\ $0.51$) while the two are level on arousal ($0.718$ vs.\ $0.72$). External numbers should be confirmed against their primary sources.}
\label{tab:sota}
\end{table}
